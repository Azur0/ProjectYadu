...
<div class="row">
    <div class="col-md-6">
        ...
        <p class="text-md-right">
            {{__('events.show_number_of_attendees', ['amount' => $event->participants->count(), 'max' => $event->numberOfPeople])}}
        </p>
        <div class="progress">
            <div class="progress-bar" role="progressbar"
                style="width: {{$event->participants->count() / $event->numberOfPeople * 100}}%" aria-valuemin="0"
        ...
        <div class="row my-1">
            ...
            <h5 class="my-auto ml-2">
                {{$participant->firstName .' '. $participant->middleName .' '. $participant->lastName}}</h5>
        </div>
        @endforeach
    </div>
    <div class="col-md-6">
        ...
        <h5>{{$event->location()->first()->postalcode}}
            {{$event->location()->first()->houseNumber}}{{$event->location()->first()->houseNumberAddition}}</h5>
        ...
        <script
            src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAuigrcHjZ0tW0VErNr7_U4Pq_gLCknnD0&callback=initMap"
            async defer></script>
        <div class="mb-5">
        ...
        <div class="mb-5">
            ...
            <div>
                <a id="share-whatsapp" class="fab fa-whatsapp"
                    style="font-size: 36px; color: #E79535; margin-right: 2%; margin-bottom: 50px; cursor: pointer; text-decoration: none;"></a>
                <a id="share-facebook" class="fab fa-facebook"
                    style="font-size: 36px; color: #E79535; margin-right: 2%; margin-bottom: 50px; cursor: pointer; text-decoration: none;"></a>
                <a id="share-twitter" class="fab fa-twitter"
                    style="font-size: 36px; color: #E79535; margin-right: 2%; margin-bottom: 50px; cursor: pointer; text-decoration: none;"></a>
                ...
                <a id="share-link" class="fa fa-link" data-toggle="modal" data-target="#confirmDeleteAccount"
                    style="font-size: 36px; color: #E79535; margin-right: 2%; margin-bottom: 50px; cursor: pointer; text-decoration: none;"></a>
            </div>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary"
                            data-dismiss="modal">{{__('events.show_share_close')}}
                        </button>
                    </div>
                </div>
            </div>
        ...
        document.getElementById("share-facebook").addEventListener('click', function() {
            ...
            window.open(url, 'popUpWindow',
                'height=500,width=700,left=400,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes'
            );
        });

        document.getElementById("share-twitter").addEventListener('click', function() {
            let url =
                `https://twitter.com/intent/tweet?text={{$event->eventName}}: ${window.location.href} %23Yadu`;
            window.open(url, 'popUpWindow',
                'height=500,width=700,left=400,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes'
            );
        });
        ...
        <div class="chat">
                        ...
                        <div v-else-if="message.user_id !== {{ Auth::id() }}">
                            <div class="message-data align-right">
                                ...
                                <span class="message-data-name"></i>
                                    @{{  message.firstName + ' ' + message.lastName }}</span> <i
            ...
            <div class="chat-message clearfix">
                <textarea name="message-to-send" id="message-to-send"
                    placeholder="{{ __('events.show_chat_typemessage') }}" rows="3" v-model="messageBox"
                    v-on:keyup.enter="postMessage"></textarea>
...
const app = new Vue({
    ...
    data: {
        ...
        messageBox: '',
        event: {
            !!json_encode($event - > getAttributes()) !!
        },
        account: {
            !!Auth::check() ? json_encode(Auth::user() - > only(['id', 'firstName', 'lastName', 'api_token'])) :
            'null'!!
        }
    },
    mounted() {
